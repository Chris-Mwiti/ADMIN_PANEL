...
const Thumbnails = ({ index, image }: { index?: number, image: string }) => {
  ...
  return (
    ...
    `}
      key={index}>
      <img src={image} alt="selected" className="size-full rounded-md" />
      <span
        className={` ${ index ? "visible" : "invisible" } size-7 rounded-full bg-black/40 absolute top-2 right-1 flex items-center justify-center hover:bg-black/60 cursor-pointer`}
        onClick={handleClose}>
